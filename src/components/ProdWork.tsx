...
function ProdWork({
  ...
  section: ProductionTypes.SectionModel & { processId: string };
  components: Array<ProductionTypes.Component>;
  machines: Array<ProductionTypes.MachienModel & { sectionId: string }>;
  maintenance: Array<MaintenanceTypes.MRequest>;
  matstore: Array<StoreTypes.MatRequestModel>;
  ...
  const [material, setMaterial] = useState<
    | { section: string; sectionId: string; item: string; quantity: number }
    | undefined
  >(undefined);
  const [active, setActive] = useState<Array<ProductionTypes.Component>>([]);

  //function
  const startProcessing = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!componentProfile) {
      ...
      (document.getElementById(`input_${index}`) as HTMLInputElement).value =
        "";
      (document.getElementById(`input_${index}`) as HTMLInputElement).readOnly =
        false;
      return;
    }
  ...
  const completeProcessing = (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      <Grid item xs={3}>
        ...
        <List>
          <ListItem sx={{ border: "solid 2px" }}>
            <ListItemText primary={"Planning"} secondary={section.planning} />
          </ListItem>
        </List>
        <List>
          <ListItem
            sx={{ border: "solid 2px" }}
            secondaryAction={
              <IconButton edge="end" value="pending" onClick={handleOpen}>
                <MoreVertIcon />
              </IconButton>
            }
          >
        ...
        <List>
          <ListItem
            sx={{ border: "solid 2px" }}
            secondaryAction={
              <IconButton edge="end" value="processing" onClick={handleOpen}>
                <MoreVertIcon />
              </IconButton>
            }
          >
        ...
        <List>
          <ListItem
            sx={{ border: "solid 2px" }}
            secondaryAction={
              <IconButton edge="end" value="stock" onClick={handleOpen}>
                <MoreVertIcon />
              </IconButton>
            }
          >
            <ListItemText primary={"Stock"} secondary={stockComponent.length} />
          </ListItem>
        </List>
      </Grid>
      <Grid item xs={9}>
                ...
                return (
                  ...
                  >
                    ...
                    <TextField
                      ...
                      label="Component No"
                      variant="filled"
                      defaultValue={machine.componentId || ""}
                      InputProps={{
                        readOnly: true,
                      }}
                    />
