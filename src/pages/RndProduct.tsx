...
function RndProduct({ Assets }: { Assets: EngineerTypes.EngineerState }) {
  const history = useHistory();

  const [name, setName] = useState("");
  const [material, setMaterial] = useState("");
  const [process, setProcess] = useState("");
  const [processes, setProcesses] = useState<Array<EngineerTypes.ProcessCode>>(
    []
  );
  const [activeProcedure, setActiveProcedure] = useState<
    { name: string; material: string; process: Array<string> } | undefined
  >(undefined);
  ...
  const addProcedure = () => {
    ...
    const prcs: Array<EngineerTypes.ProcessCode> = processes.map((prs) => {
      return {
        id: prs.id,
        code: prs.code,
      };
    ...
    const procedure: EngineerTypes.Procedure = {
      name,
      material,
      processes: prcs,
    };
  ...
  const delProcedure = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    <Box sx={{ maxWidth: "800px", mx: "auto", mt: 15 }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={7}>
          ...
          <Box my={2} sx={{ display: "flex" }}>
            <IconButton sx={{ mx: "auto" }} size="large" onClick={addProcess}>
              <ArrowCircleDownIcon />
            </IconButton>
          </Box>
          <Box>
            <List subheader={<ListSubheader>Process Flow</ListSubheader>}>
          ...
          <Box display="flex" justifyContent="flex-end" p={1}>
            ...
            <Button sx={{ mx: 1 }} variant="contained" onClick={addProcedure}>
              Submit
            </Button>
          </Box>
        </Grid>
      ...
      >
        <Box sx={style}>
          ...
          <List>
            {activeProcedure?.process &&
              activeProcedure?.process.map((prcs, idx) => {
                return (
