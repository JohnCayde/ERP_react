...
function MainRequestPen({requests,sections}) {

    const PendingRequests = requests.filter(request=>request.status=="reviewed").map((request=>{
        const idComp = request.id.split("-")
        const sectionProfile = sections.find(section=>section.id == request.section)
        ...
        <Box my={1}>
            <Box>
                ...
                <Typography variant="caption" display="block" gutterBottom>
                    （Requests from production for maintenance issue that reviewed by maintenance)
                </Typography>
            </Box>
            ...
            <ModalMainRequest mode="complete" request={activeRequest} open={open} handleClose={handleClose}/>
            <Modal
                open={complete}
