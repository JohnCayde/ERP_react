...
function StoreMatPurReqeust({allRequests, items}) {
    //data processing
    const requests = allRequests.filter(request=>request.status == "pending").map((request)=>{
        ...
        <Box my={1}>
            <Box>
                ...
                <Typography variant="caption" display="block" gutterBottom>
                    （Request purchasing department to purchase material)
                </Typography>
            </Box>
            <Box sx={{ my: 2 }}>
            ...
            <ModalRequestDetails request={activeRequest} open={open} handleClose={handleClose}/>
            <ModalItemRequest department={"Material Store"} items={items} open={openRequest} handleClose={handleCloseRequest}/>
        </Box>
    )
}

export default StoreMatPurReqeust
